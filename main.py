...
def highlightbox():
    for k in range(2):
        pygame.draw.line(Window, (0, 0, 0), (x * diff, (z + k)*diff), (x * diff + diff , (z + k)*diff), 3)
        pygame.draw.line(Window, (0, 0, 0), ( (x + k)* diff, z * diff ), ((x + k) * diff, z * diff + diff), 3) 
    ...
    for i in range (9):
        for j in range (9):
            if defaultgrid[i][j] != 0:
                pygame.draw.rect(Window, (255, 255, 255), (i * diff, j * diff, diff + 1, diff + 1))
...
# Raising error when wrong value is entered
def raiseerror():
